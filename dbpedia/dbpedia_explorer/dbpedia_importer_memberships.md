...
* http://dbpedia.org/ontology/almaMater pour 113 cas
  * "schools that they attended (en)" comme l'indique la notice de l'ontologie
* http://dbpedia.org/ontology/employer pour 54 cas
  * la définition de la propriété dans l'ontologie de DBpedia ne donne pas de précisions, juste "employer" mais en cible de la propriété il y a des organisations: *rdfs:range -> dbo:Organisation* et on peut donc utiliser pour les appartenances.
...
On rédige la requête qui récupère les données:
    ...
    WHERE { 
    dbr:List_of_war_correspondents ?p ?person.
    ?person a dbo:Person;
            dbo:birthDate ?birthDate ;
...
### Média ou agence (comme appartenance)

On rédige la requête qui récupère les données:

    PREFIX dbr: <http://dbpedia.org/resource/>
    PREFIX dbo: <http://dbpedia.org/ontology/>
    SELECT DISTINCT ?person (?employer AS ?organisation)
    ("employer" as ?membership_type) (3 AS ?fk_sparql_query)
    (STR("Importation 29 janvier 2026") as ?metadata)
    WHERE { 
    dbr:List_of_war_correspondents ?p ?person.
    ?person a dbo:Person;
            dbo:birthDate ?birthDate ;
